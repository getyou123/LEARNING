...
public class EmpTest {
    ...
    @Test
    public void testGetEmpListByCondition() {
        // 获取 sqlSession 对象
        SqlSession sqlSession = SqlSessionUtil.getSqlSession();
        // 获取Mapper代理实现类对象
        EmpMapper EmpMapper = sqlSession.getMapper(EmpMapper.class);
        Emp emp = new Emp();
        emp.setEmpName("admin1");
        emp.setSex("男");
        List<Emp> empListByCondition = EmpMapper.getEmpListByCondition(emp);
        System.out.println(empListByCondition);
    }

    @Test
    public void testInsertMultiEmps() {
        // 获取 sqlSession 对象
        SqlSession sqlSession = SqlSessionUtil.getSqlSession();
        // 获取Mapper代理实现类对象
        EmpMapper EmpMapper = sqlSession.getMapper(EmpMapper.class);
        Emp emp = new Emp();
        emp.setEmpName("insert1");
        emp.setSex("男");
        emp.setEmpId(5);

        Emp emp1 = new Emp();
        emp1.setEmpName("insert2");
        emp1.setSex("男");
        emp1.setEmpId(5);

        List<Emp> userList = new LinkedList<Emp>();
        userList.add(emp);
        userList.add(emp1);

        EmpMapper.insertMultiEmps(userList);

    }


    @Test
    public void testDeleteMultiEmps() {
        // 获取 sqlSession 对象
        SqlSession sqlSession = SqlSessionUtil.getSqlSession();
        // 获取Mapper代理实现类对象
        EmpMapper EmpMapper = sqlSession.getMapper(EmpMapper.class);
